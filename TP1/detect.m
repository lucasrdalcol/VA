...
if ~isempty(cameraDetections)
    [~,idx] = sort(cameraDetections(:,1)); % sort just the first column 
    sorted_cameraDetections = cameraDetections(idx,:); % reorganize the array of coordinates
end

if ~isempty(cameraBicycle)
    [~,idx] = sort(cameraBicycle(:,1)); % sort just the first column 
    sorted_cameraBicycle = cameraBicycle(idx,:); % reorganize the array of coordinates
end

filtered_sorted_radarDetections = sorted_radarDetections;
    ...
    if excludeCars == true
        for i = 1:size(detectedStopCars, 2)
            for n = 1:size(detectedStopCars{i}, 1)
                ds = filtered_sorted_radarDetections - detectedStopCars{i}(n, :);
                ds = sqrt(ds(:, 1).^2 + ds(:, 2).^2);
        
                idxs = find(ds < distTreshCar);
                
                if numel(idxs) >= numMinDetections
                    filtered_sorted_radarDetections(idxs, :) = [];
                    excludeCars = false;
                end
            end
        end
...
ylabel('Y (m)')
hold on
plot(filtered_sorted_radarDetections(:, 1), filtered_sorted_radarDetections(:, 2), 'ro')
    ...
    if excludeBicycles == true
        for i = 1:size(detectedBicyles, 2)
            for n = 1:size(detectedBicyles{i}, 1)
                ds = filtered_sorted_radarDetections - detectedBicyles{i}(n, :);
                ds = sqrt(ds(:, 1).^2 + ds(:, 2).^2);
