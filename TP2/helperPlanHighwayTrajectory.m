...
maxCurvature    =  0.5; % 1/meters, or radians/meter
...
costTS = exampleHelperEvaluateTSCost(allTS,allDT,laneWidth,speedLimit,...
    speedWeight, latDevWeight, timeWeight, rightLaneWeight);
...
if numel(optimalTrajectory) == 0
    ...
    costST = exampleHelperEvaluateTSCost(termStatesST,timesST,laneWidth,speedLimit,...
    speedWeight, latDevWeight, timeWeight, rightLaneWeight);

    egoFrenetState = global2frenet(refPath,egoState);
    [~,stopGlobalTraj] = connect(connector,egoFrenetState,termStatesST,timesST);

    % Eliminate trajectories that violate constraints.
    isValid = exampleHelperEvaluateTrajectory(stopGlobalTraj,maxAcceleration,maxCurvature,minVelocity);

    % Determine evaluation order.
    [~, idx] = sort(costST);
    optimalTrajectory = [];

    stopTrajectoryEvaluation = nan(numel(isValid),1);

    % Check each trajectory for collisions starting with least cost.
    for i = 1:numel(idx)
        if isValid(idx(i))
            % Update capsule list with the ego object's candidate trajectory.
            egoPoses.States = stopGlobalTraj(idx(i)).Trajectory(1:collisionCheckResolution:end,1:3);
            updateEgoPose(capList,1,egoPoses);
            ...
            if all(~isColliding)
                % If no collisions are found, this is the optimal.
                % trajectory.
                stopTrajectoryEvaluation(idx(i)) = 1;
                optimalTrajectory = stopGlobalTraj(idx(i)).Trajectory;
                break;
            else
                stopTrajectoryEvaluation(idx(i)) = 0;
            end
        end
    end

    trajectoryList = stopGlobalTraj;
    for i = 1:numel(trajectoryList)
        trajectoryList(i).Evaluation = stopTrajectoryEvaluation(i);
        trajectoryList(i).IsValid = isValid(i);
    end
end
...
function costs = exampleHelperEvaluateTSCost(terminalStates, times, laneWidth, speedLimit, speedWeight, latWeight, timeWeight, rightLaneWeight)
...
costs = latCost+timeCost+speedCost+rightLaneCost;
end
